...
```bash
# Full grid (defaults: variants="noble:24.04 resolute:26.04" modes="colocated separate")
./test/run-matrix.sh
# Single smoke combo (one variant x one mode), e.g. resolute/separate:
MATRIX_VARIANTS="resolute:26.04" MATRIX_MODES="separate" ./test/run-matrix.sh
```

`run-matrix.sh` guarantees a per-combo `teardown.sh` even on mid-pipeline failure (so the next combo's `setup.sh` never collides on "VM already exists"), continues on a combo failure, logs each combo to `test/logs/<codename>-<mode>.log`, and prints a summary table. Run it detached with `</dev/null`.
...
| `test_export_fsid` | share (`fsid=101`) and homes (`fsid=100`) export files carry the expected `fsid=`, and the kernel accepted each export (`exportfs -v`) |
...
| `test_client_dns_registration` | client's A/PTR are registered in the DC's AD zone (default: explicit `sssd_register_dns`); DC resolves the client by name |
...
| `test_sssd_socket_activation` | no `services` line in `sssd.conf`, nss/pam responder sockets enabled, no `sssd-*.socket` failed, `sssd.service` active — on client + DC + storage |
| `test_dc_pam_mkhomedir` | DC's `pam_mkhomedir` hook is present, is the stock (option-less) pam-auth-update-managed line (legacy `skel=`/`umask=` form absent), and the `mkhomedir` profile is registered with pam-auth-update via debconf |
...
**Deprovision coverage.** The `run-tests.sh` suites above assume a live, provisioned environment, so deprovision is verified separately by `test/deprovision-tests.sh` (run after `run-tests.sh`, before `teardown.sh`). It runs `deprovision-linux.yml` against the client and (in separate mode) `deprovision-nfs-server.yml` against the storage host, then asserts the teardown: `sssd` stopped+disabled, every `sssd-*.socket` stopped+disabled, `sssd.conf` removed, `automount:`/`sudoers:` SSS nsswitch routing gone, sshd snippet gone, `realm list` empty, and (storage) `nfs-server`/`rpc-svcgssd` stopped, autofs unmasked, DC root key revoked. DNS A-record removal is a soft (warn) check since the playbook does it best-effort.
...
- **SSSD config decisions** (both DC and client templates): `ad_gpo_access_control = permissive` — logs GPO denials but doesn't enforce them, avoiding lockouts from absent/misconfigured GPOs in a Samba AD domain. `ldap_referrals = false` — referral chasing costs round trips and reveals no extra data in a single-forest Samba domain. `dyndns_update = false` on the DC's own `sssd.conf` (samba-ad-dc owns the DC's records) — see "Member DNS registration" below for how clients/NFS servers get their records.
- **Responders run via socket activation** — neither `sssd.conf` template emits a `services` line. On systemd hosts SSSD's responders are socket/D-Bus activated (`sssd-<service>.socket` units); per `sssd.conf(5)`, listing a responder in `services` **and** having its socket enabled trips `sssd_check_socket_activated_responders` (exit-code 17), so every conflicting socket fails to listen at boot (the loud "Misconfiguration found for the '<svc>' responder" / "Failed to listen on sssd-<svc>.socket" journald noise). The `sssd-client` role asserts the required socket set in `tasks/services.yml` (nss, pam, +sudo/ssh/autofs per the `sssd_enable_*` flags; `pac` left at its package default since the AD provider auto-uses it via `implicit_pac_responder`); the `samba-dc` role enables `sssd-nss.socket`/`sssd-pam.socket` in `tasks/sssd.yml`. The per-responder domain config (`sudo_provider`, `autofs_provider`, SSH-key attrs) is read by the responders regardless of how they were started. The deprovision playbooks disable+stop the `sssd-*.socket` set so systemd can't re-activate a responder against a removed `sssd.conf`. `test_sssd_socket_activation` guards the "sockets correctly up" side and `test/deprovision-tests.sh` guards the "correctly down after deprovision" side.
...
- **krb5 realm mapping for site-named servers** (`sssd_krb5_realm_map`, default `{}`): when the DC or an NFS server is reached by a DNS name that isn't under the AD realm's domain (e.g. an externally-owned site FQDN `host.uhnres.utoronto.ca` serving realm `AD.GENETLAB.INTERNAL`), krb5 derives the realm from the DNS domain and `rpc.gssd` requests `nfs/<host>@<wrong-realm>` → *"Server not found in Kerberos database"*, so NFSv4 `sec=krb5p` home/share mounts fail with "access denied by server" **even though `getent`/user lookups work** (SSSD has its own credential path; the failure is only in the NFS GSS upcall). The role ships this (plus `rdns = false` / `dns_canonicalize_hostname = false`) as a **`/etc/krb5.conf.d/samba-ad.conf` drop-in** (`krb5-samba-ad.conf.j2`, notifies `Restart rpc-gssd`), not by editing `/etc/krb5.conf` in place — both roles ensure `includedir /etc/krb5.conf.d/` first (the DC's Samba-generated `krb5.conf` lacks it; the stock client one already has it), and the DC ships its own `krb5.conf.d/samba-dc.conf` with `rdns = false`. Set `sssd_krb5_realm_map` in `group_vars/linux_clients.yml` to each server's site FQDN → realm; prefer specific FQDNs over a broad `.suffix` so you don't hijack other Kerberos services in that domain (the stock krb5.conf commonly maps whole domains, e.g. `.utoronto.ca = UTORONTO.CA`). Diagnose with `KRB5_TRACE=/dev/stdout kvno -S nfs <server-fqdn>` (hostbased, mimics `rpc.gssd`) vs `kvno nfs/<fqdn>@<REALM>` (literal) — a realm mismatch between them is this bug.
...
- **Member DNS registration** (`sssd-client/tasks/dns-register.yml`): `realm join`/adcli creates the AD computer object but **not** a DNS A record, so the `sssd-client` role registers each domain member's A/PTR in the DC's AD zone. Two mechanisms, mutually exclusive by default:
  - **`sssd_register_dns` (default ON)** — explicit registration run **on the DC** via delegated `samba-tool dns` (`no_log`; addressed at `127.0.0.1`, since the commands already run on the DC and its inventory name is the least reliable thing during an address change). No client-side Kerberos, so it works on **split-identity** sites where the DC's network name is outside the AD realm and a client GSS-TSIG update fails with *"Server not found in Kerberos database"*. The PTR value is the host's **AD name** (`hostname + sssd_domain`), never its (possibly foreign) DNS FQDN. This is the robust default; **NFS storage hosts** depend on it (the `sssd-client` role runs before `nfs-server`, so the storage host is already registered — `nfs-server` no longer registers DNS itself). **Query-then-*update*, not query-then-add**: the task publishes the host's current address and deletes every A/PTR value that isn't it. The earlier add-if-missing form meant a member whose DHCP lease moved kept advertising a dead address in AD forever — silently, because "the record exists" reads as success.
  - **`sssd_dyndns_update` (default OFF)** — SSSD dynamic DNS: the host self-registers via GSS-TSIG (`dyndns_iface` pinned for determinism). Self-heals on IP change but requires a working host→DC GSS-TSIG path, so only enable it on **non-split-identity** networks. Turning it on auto-disables explicit registration (`sssd_register_dns: "{{ not (sssd_dyndns_update | bool) }}"`).
  - The DC keeps `dyndns_update = false` (samba-ad-dc owns its own records). `deprovision-linux.yml` best-effort deletes the client's A/PTR on leave (records don't self-clean); `test_client_dns_registration` verifies the DC resolves the client by name regardless of mechanism.
...
- **PAM is managed exclusively through `pam-auth-update`** (the stock `mkhomedir` profile from `libpam-modules`) — never by editing `/etc/pam.d/common-*` directly. A hand-inserted line lands inside pam-auth-update's managed block and corrupts its checksum tracking, after which pam-auth-update sees "local modifications" and refuses to cleanly manage the stack on package upgrades. The `samba-dc` role (`tasks/nss_pam.yml`) enables `mkhomedir` unconditionally (the DC provisions homedirs locally); the `sssd-client` role toggles it per `sssd_homedir_mode`. The stock profile emits `session optional pam_mkhomedir.so` with no options, which is functionally identical to the old hand-inserted line since `skel=/etc/skel`/`umask=0022` are the module's built-in defaults. `nss_pam.yml` also carries a one-time removal task (regexp anchored to the legacy option string, so it can't touch the managed line) that strips the old hand-inserted line from already-provisioned DCs, restoring pam-auth-update's checksum. `test_dc_pam_mkhomedir` in `run-tests.sh` guards this.
...
- **Client DNS-routing needs the DC's IP** (`sssd-client/tasks/dns-routing.yml`, imported by `install.yml` and reused standalone by `repair-dc-address.yml`): the role writes `DNS=<ip>` into `/etc/systemd/resolved.conf.d/samba-ad.conf`, which requires an address, not a name. It resolves the IP via `sssd_dc_ip` → the DC's gathered `ansible_default_ipv4` → the DC's `ansible_host` → the raw inventory name, and asserts the result is IPv4. With an FQDN-based inventory, a **client-only** run (e.g. `provision-linux-sssd.yml` limited to clients) gathers no DC facts and falls through to the FQDN, tripping the assert. `provision-linux-sssd.yml` therefore runs a `hosts: dc` fact-gathering play first so unlimited runs resolve automatically; for `--limit`ed or otherwise DC-less runs, set `sssd_dc_ip` in `group_vars/domain_members.yml`. `Domains=` routes the AD zone plus the reverse zones of **both** the DC's subnet and the host's own — deriving it from the DC's address alone silently un-routed the client's own reverse zone whenever the two ended up on different /24s. After writing the drop-in the file re-queries `_ldap._tcp.<domain>` and **fails the play** when it doesn't resolve, so a stale address is named as the cause instead of surfacing three tasks later as a `realm discover` failure.
- **DC address changes are self-healing** — this is the one place the stack stores an address rather than a name, so an address change used to break the domain in a way that pointed everywhere except at DNS. Every address- and name-sensitive task is now query-then-repair on each run: `samba-dc/tasks/hosts-file.yml` (writes and asserts the `/etc/hosts` binding; the old check lived in `assertions.yml` and only rejected `127.0.0.1`, so a stale-but-routable address passed), `dns-self-heal.yml` (the DC's own A record + zone apex, pruning values left by a previous address, then `samba_dnsupdate --verbose --all-names` for the SRV/`_msdcs` set — gated so a steady-state run reports no change), `reverse-dns.yml` (reverse zone for the *current* subnet, PTR value repaired not just created), `autofs_seed.yml` (re-points the seeded `auto.home` wildcard; per-share `auto.shares` entries are operator data and are left alone), and `sssd-client/tasks/dns-register.yml`. `playbooks/repair-dc-address.yml` runs exactly that subset via `include_role ... tasks_from:` (with `public: true` so role defaults reach the play's own tasks) and asserts `sam.ldb` exists before touching anything — a site hostname that encodes the old address may now resolve to whatever machine inherited the lease.
- **`samba_dc_canonical_fqdn` (default `""` → `<hostname>.<realm>`)** is the DC's canonical name for `/etc/hosts`, the PTR value, the `nfs/<fqdn>` SPN and keytab principal, the seeded autofs targets, and `NFS_SERVER`/`NFS_HOMES_SERVER` in `samba-mgmt.conf`. These all used `ansible_facts['fqdn']`, which on a split-identity site is the **site** FQDN — so a site name that encodes the address (`opennet-33-37.uhnres.utoronto.ca`) took every one of them down together when the lease moved, and needed a matching `sssd_krb5_realm_map` entry besides. The AD name is served by the DC itself and survives. `samba_dc_fqdn` (the resolved value) is a lazily-evaluated role default, so it is available to every task file regardless of include order — unlike a `set_fact`, which would be stale in the same run that repairs `/etc/hosts`.
- **Deprovision is thorough**: `deprovision-linux.yml` / `deprovision-nfs-server.yml` don't just `realm leave` — they stop/disable `sssd.service`, stop/disable every `sssd-*.socket` (so socket activation can't respawn a responder against the about-to-be-removed config), remove `sssd.conf`, the `/etc/systemd/resolved.conf.d/samba-ad.conf` DNS routing drop-in (else the client keeps pointing DNS at a dead DC), the sshd `AuthorizedKeysCommand` snippet, and the autofs/sudoers `nsswitch.conf` lines, restarting `systemd-resolved`/`sshd` afterward. The NFS-server variant additionally unmasks autofs, revokes the DC's root key from `authorized_keys`, and best-effort deletes the host's DNS A/PTR on the DC. Both deprovision playbooks are exercised end-to-end by `test/deprovision-tests.sh`.
...
- **idmapd.conf** must be deployed on both NFS server and clients with matching `Domain = <realm>` for consistent UID/GID mapping. All three roles (`samba-dc`, `nfs-server`, `sssd-client`) deploy the **same** `idmapd.conf.j2`, which lives once in `common/templates/` and is referenced via `role_path/../common/templates/` so the copies can't drift. (`homes.exports.j2` is shared the same way.)
...
- **Time sync uses whichever daemon the image ships.** The role detects the time daemon via `service_facts` and configures `systemd-timesyncd` where present (22.04/24.04, Debian 12) or `chrony` otherwise (Ubuntu 26.04 cloud images ship chrony and no systemd-timesyncd). timesyncd is configured via `/etc/systemd/timesyncd.conf`; chrony via an `/etc/chrony/conf.d/samba-ntp.conf` drop-in of `server <host> iburst` lines. Both are driven by `samba_ntp_servers`. Neither package is force-installed over the image default. The role keeps `systemd-resolved` **enabled** and routes DNS to Samba via a `/etc/systemd/resolved.conf.d/samba-dc.conf` drop-in (`DNS=127.0.0.1`, `Domains=<realm> ~.`) rather than clobbering `/etc/resolv.conf` — there is no port conflict because Samba binds `127.0.0.1:53` (via `interfaces = lo <iface>` + `bind interfaces only`) while resolved's stub listens on the different address `127.0.0.53:53`. `/etc/resolv.conf` stays the resolved stub symlink; a migration task re-links it on DCs provisioned by the old static-file/disabled-resolved code path. The historical `ntpsigndsocket` setup for AD-aware time signing is not used.
...
- **Provisioning idempotency** uses both `.samba_domain_provisioned` marker file (under `samba_statedir`) and `sam.ldb` presence — the marker alone could be stale if `/var/lib/samba` was wiped.
...
- **Sudo rules** are stored as `sudoRole` objects in `OU=SUDOers` in AD (the sudo project's `sudoers.ldap(5)` convention). This requires the sudo LDAP schema extension, applied during DC provisioning (`samba_enable_sudo_schema: true`). **The `sssd-client` template pins `ldap_sudo_search_base = OU=SUDOers,<domain-DN>` explicitly** (`sssd.conf.j2`, gated on `sssd_enable_sudo`): letting SSSD auto-discover the base produced a full refresh that downloaded **0** rules on a real Samba DC, so AD sudo rules silently never reached clients (`sudo -l` always empty) even though the rules, the user's group membership, and the host filter were all correct. Symptom/diagnosis: `sssctl domain-status` is Online and `getent` works, but the backend log shows the `SUDO Full Refresh` running and `Received 0 sudo rules`; the SSSD sudo cache (`/var/lib/sss/db/cache_<domain>.ldb`) stays empty. Once rules are cached, `%group` matching is case-insensitive, so `%Domain Admins` and `%domain admins` both work.
...
- **Home directory ownership**: `samba-user.sh add` creates `/home/ad/<user>` owned by the user, mode 0700, and seeds it from `/etc/skel` on the homes host like `useradd -m` (`cp -aT /etc/skel`, skipped gracefully if the host has no `/etc/skel`; the chown is recursive so the skeleton files end up user-owned, while `chmod 0700` locks only the top directory). Before the chown it runs `sss_cache -u <user>` on the homes host and waits for the account to resolve via NSS — a recently-deleted same-named account can otherwise linger in SSSD's cache and the new home would be chowned to the dead uid. If the home already exists it compares **numeric** uids (not names) and warns when they differ (a stale uid→name mapping could make a name comparison pass while ownership is wrong). `delete` never removes home data; it logs where the data was preserved.
...
- **Idempotency gates**: DC provisioning checks the `.samba_domain_provisioned` marker AND `sam.ldb`. Client join checks `realm list` output. Sudo schema is gated by a `.sudo_schema_applied` marker plus a live `ldbsearch` for `cn=sudoRole` in the schema.
...
- **Facts are referenced via `ansible_facts['name']`, never the top-level `ansible_<name>` alias.** Both `ansible/ansible.cfg` and `test/ansible.cfg` set `inject_facts_as_vars = False`, which opts into the future ansible-core default (the top-level injection is deprecated and removed in 2.24) and silences the `INJECT_FACTS_AS_VARS` deprecation warning. With it off, `ansible_hostname` / `ansible_fqdn` / `ansible_default_ipv4` / `ansible_distribution` etc. are **undefined** — use `ansible_facts['hostname']`, `ansible_facts['fqdn']`, `ansible_facts['default_ipv4'].address`, `ansible_facts['distribution']`, and so on (including inside `hostvars[...]`, e.g. `hostvars[groups['dc'][0]].ansible_facts['default_ipv4'].address`). This does **not** apply to connection variables (`ansible_host`, `ansible_user`, `ansible_port`, `ansible_python_interpreter`, ...) or to `ansible_managed`, which are not gathered facts and remain top-level. Audit new code with `grep -rEn 'ansible_[a-z_]+' roles playbooks inventory | grep -v ansible_facts` and confirm every remaining hit is a connection var.
