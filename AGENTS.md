...
| `test_client_dns_registration` | client self-registers its A/PTR in the DC's AD zone via SSSD dynamic DNS (`sssd_dyndns_update`); DC resolves the client by name |
...
- **SSSD config decisions** (both DC and client templates): `ad_gpo_access_control = permissive` — logs GPO denials but doesn't enforce them, avoiding lockouts from absent/misconfigured GPOs in a Samba AD domain. `ldap_referrals = false` — referral chasing costs round trips and reveals no extra data in a single-forest Samba domain. `dyndns_update` — controlled by `sssd_dyndns_update` (default `false` in the `sssd-client` role): **clients** set it `true` (in `group_vars/linux_clients.yml`) so they self-register their A/PTR in the DC's AD-integrated zone via GSS-TSIG (the DC can then resolve members by name; `dyndns_iface` is pinned to the primary interface so a multi-homed host can't publish an unreachable address). **NFS servers keep it `false`** and register explicitly via `nfs-server/tasks/dns-register.yml` (they need synchronous, deterministic records for `rpc.gssd` reverse resolution and DC→host SSH), and the **DC's own** `sssd.conf` keeps it `false` (samba-ad-dc owns the DC's records). `deprovision-linux.yml` best-effort deletes the client's A/PTR on leave since DDNS doesn't self-clean.
