...
- **`nfs-common` is a generated systemd unit**, not the packaged one. On Ubuntu the role removes `/usr/lib/systemd/system/nfs-common.service` (a `/dev/null` mask on 22.04/24.04/26.04; the removal is a harmless no-op on releases that don't ship the mask) and writes a real oneshot unit in `/etc/systemd/system/`. `rpc-svcgssd` is tolerated as missing because some distros merge it into `nfs-server.service`. **Before starting `rpc-gssd`, the role explicitly starts `run-rpc_pipefs.mount`**: `rpc.gssd` opens the `pipefs-directory` from `/etc/nfs.conf` (`/run/rpc_pipefs`), but on Ubuntu 22.04 that mount is left inactive at boot (the legacy `/var/lib/nfs/rpc_pipefs` is mounted instead and `rpc_pipefs.target` reports active without it), so rpc-gssd would abort with `opendir(/run/rpc_pipefs) failed`. Starting the mount unit is a no-op on 24.04/26.04/Debian 12.
...
- **DC packages are distribution+version specific.** `tasks/main.yml` loads `vars/{{ distribution }}-{{ version }}.yml` in preference to `vars/{{ distribution }}.yml`. 24.04/26.04 and Debian 12 use the `samba-ad-dc` metapackage; **Ubuntu 22.04 has no such package** (`vars/Ubuntu-22.04.yml`) — the AD DC daemon and `samba-ad-dc.service` ship in `samba`, `samba-tool` in `samba-common-bin`, and `winbind` must be installed explicitly (the AD DC daemon forks an internal `/usr/sbin/winbindd` and aborts without it; the `winbind` *service* is still masked). The DC-side SSSD packages (`samba_dc_sssd_packages`) are installed in `install.yml` **before** `smb-time-dns.yml` switches the resolver to the local Samba DNS, so their dependency downloads use the image's original working resolver — post-switch external resolution through the Samba forwarder is unreliable mid-provision and breaks apt on releases (like jammy) whose base image doesn't already satisfy the deps.
