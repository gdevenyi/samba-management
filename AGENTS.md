...
- **Member DNS registration** (`sssd-client/tasks/dns-register.yml`): `realm join`/adcli creates the AD computer object but **not** a DNS A record, so the `sssd-client` role registers each domain member's A/PTR in the DC's AD zone. Two mechanisms, mutually exclusive by default:
  - **`sssd_register_dns` (default ON)** — explicit registration run **on the DC** via delegated `samba-tool dns add` (query-then-add, idempotent, `no_log`). No client-side Kerberos, so it works on **split-identity** sites where the DC's network name is outside the AD realm and a client GSS-TSIG update fails with *"Server not found in Kerberos database"*. The PTR value is the host's **AD name** (`hostname + sssd_domain`), never its (possibly foreign) DNS FQDN. This is the robust default; **NFS storage hosts** depend on it (the `sssd-client` role runs before `nfs-server`, so the storage host is already registered — `nfs-server` no longer registers DNS itself).
